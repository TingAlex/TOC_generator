...
- `--write` 不做整本源/目标双扫。它先创建在线本和直属分区，再按分区逐页读取 `piAll`：已有前缀
  逐页复核，新页在复制事务内两次复读。源页摘要传入复制事务复用，避免同一源页重复读取大型 Base64 XML。
  收尾只复查页数、标题和顺序；XPS/图片正文已经在各单页同步屏障内验证。
...
`toc-onenote-batch` 只负责编排，不改变上述单页事务边界。它读取 `version=1` JSON 清单，按数组顺序为
每本书启动一个独立的 `toc-onenote-copy-online` 子进程，等待退出后才进入下一本，因此多本之间也不会
交错操作 OneNote UI。`--detach` 用 Windows 隐藏分离进程运行，不依赖发起它的终端或 AI 会话存活。

可靠性边界：

- `.lock` 使用 Windows 非阻塞文件区间锁，保证同一任务同一时刻只有一个调度器。
- `.state.json` 采用临时文件 + `os.replace` 原子更新，保存 plan SHA-256、当前书、尝试次数、退出码和
  最近一行进度；plan 改变或 dry-run/write 模式改变时拒绝复用旧状态。
- `--status` 同时非阻塞探测 `.lock`。若文件状态为 `running/retrying`、但锁无人持有，则报告
  `effective_status=interrupted`；下次执行取得锁后记录一次 recovery 并从 OneNote 的真实前缀续跑。
- `.log` 只追加子进程完整输出，状态损坏或人工排障时仍保留审计记录。
- 单本失败会有限重试；底层命令每次都重新扫描源/目标并验证原生副本前缀，所以进程崩溃或机器重启后
  不依赖内存计数，也不会因为状态文件写了“第 N 页”就盲目跳页。
- 已标记完成的书不会再次启动；整批完成后状态为 `complete`。状态仅用于调度，页面真实性始终由
  OneNote COM 复读的 XPS、图片摘要和版面校验决定。
- `scripts/start_onenote_batch_task.ps1` 把调度器注册到当前用户的 Windows 任务计划程序交互会话；
  进程异常消失时由 Task Scheduler 最多重启 3 次。它绕开终端、IDE 或 AI 工具对子进程生命周期的影响，
  同时仍由 `.lock` 防止重复实例。
- 原生对话框只暴露显示名称。若本地源本和在线目标同名、且直属分区也同名，须给本地源本设置不同的
  `nickname`（如 `_本地`）；层级 `name` 和磁盘目录保持不变，命令参数仍使用原名。
